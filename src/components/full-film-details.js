...
const createCommentElement = (count) => {
	
	const comments = generateComments(count);
  return comments.map(({text, author, date, emoji} = comments) => {
...
const createFullFilmDetailsTemplate = (film) => {
  ...
  const {title, poster, rating, altTitle, ageRating, director, writer, actor, date, country, duration, genre, description, comments} = film;
const comment = createCommentElement(comments);
...
export default class FullFilmDetails {
  constructor(film) {
    this._film = film;
